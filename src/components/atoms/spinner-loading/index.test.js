...
describe("Loading and spinner Component", () => {
    ...
    it("renders spinner loading props isloading false <Loading/>", () => {
        expect(mount(<Loading isLoading={false}></Loading>).prop("isLoading")).toEqual(false);
    });
    it("renders spinner loading and snapshot element  <spinner/>", () => {
        expect(shallow(spinner).prop("color")).toEqual("lightGreen");
    });
    it("renders spinner loading and snapshot element  <spinner/>", () => {
        expect(
            mountWithTheme(<StyleSpinner color="lightGreen"></StyleSpinner>)
                .find("div")
                .prop("color"),
        ).toEqual("lightGreen");
    });
